            ...
            foreach ($_FILES["files"]["error"] as $key => $error) {
                if ($error == UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tmp_name, $file_path) === true) {
                        ...
                        $image->resizeImage($config[THUMBNAIL_WIDTH], $config[THUMBNAIL_HEIGHT],
                            Imagick::FILTER_LANCZOS, 1.0, true);
...
function ProcessDlRequest()
{
    ...
    switch ($config[PREMAILER]) {
        ...
        case 'inlinestyle':
            ...
            $htmldoc = new \InlineStyle\InlineStyle($_POST["html"]);
    ...
    for ($i = 0; $i < $num_full_pattern_matches; $i++) {
        if (stripos($matches[1][$i], "/img/?src=") !== false) {
            ...
            if (preg_match('#/img/\?src=(.*)&amp;method=(.*)&amp;params=(.*)#i', $matches[1][$i],
                    $src_matches) !== false
            ) {
                ...
                $html = str_ireplace($matches[1][$i],
                    $config[BASE_URL] . $config[STATIC_URL] . urlencode($static_file_name), $html);
